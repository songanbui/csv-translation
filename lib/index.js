...
let _translate = function (contentFile, dictionaryFile) {
  ...
  return Promise.all(parsePromises).then(function (results) {
    ...
    let dictionary = results[1].data;             console.log(dictionary);
...
let _findInDictionary = function (value, dictionary) {
  ...
  if (typeof value === 'string' && value !== '') {
    ...
    // Find the best matching dictionary translation
    let translatedCharCount = 0;
    for (let r = 0; r < dictionary.length; r++) {
      let current = dictionary[r][0];
      if ((typeof current === 'string') && value.indexOf(current) !== -1) {
        if (current.length > translatedCharCount) {
          result = dictionary[r][1];
          translatedCharCount = current.length;
        }
      }
    }
  }
